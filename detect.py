...
class YoloV7:
    ...
    def detect(self, save_ori_img=False, only_ball=False, mark_no_count=False):
        ...
        while True:
            if source == "":
                user_source = input("請輸入source的路徑:")
            else:
            ...
            # create directory
            save_img = not opt.nosave and not user_source.endswith(".txt")  # save inference images
            webcam = (
                user_source.isnumeric()
                or user_source.endswith(".txt")
                or user_source.lower().startswith(("rtsp://", "rtmp://", "http://", "https://"))
            )
            camera = bool(re.search("^/dev/video\d+", user_source))  # true if use camera
            ...
            if camera:
                ...
                dataset = LoadCamera(user_source, img_size=imgsz, stride=stride, step=opt.step)
                process_video = ProcessVideos()
            elif webcam:
                view_img = check_imshow()
                cudnn.benchmark = True  # set True to speed up constant image size inference
                dataset = LoadStreams(user_source, img_size=imgsz, stride=stride, step=opt.step)
            else:
                dataset = LoadImages(user_source, img_size=imgsz, stride=stride, step=opt.step)
                    ...
                    if opt.draw_ball_path != "":
                        ...
                        if len(det):
                                ...
                                if cls == 1:  # person boundaries
                                    if opt.person_left_boundary != "" and opt.person_right_boundary != "":
                                        ...
                                        right_numerator, right_denominator = map(
                                            int, opt.person_right_boundary.split("/")
                                        )
                            ...
                            if (
                                ...
                                xywh = (
                                    (xyxy2xywh(torch.tensor(most_confidence_ball_xyxy).view(1, 4)) / gn)
                                    .view(-1)
                                    .tolist()
                                )  # normalized xywh
                                line = (
