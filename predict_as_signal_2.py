...
def main():
    ...
    window_usecols = []
    for col in usecols:
        if col in ["Curvature", "a_2", "a_1", "a_0"]:
            window_usecols.append(col)
        else:
            window_usecols.append(col + "_X")
            window_usecols.append(col + "_Y")
    ...
    record_ball_df = pd.DataFrame(columns=["Frame", "Visibility", "X", "Y", "Time"])  # 球點預測
    record_pose_df = pd.DataFrame(columns=usecols)

    total_time = 0

    start = time.time()
    ## Pose data preprocessing
    if not os.path.exists(f"{record_pose_path}/{video_name}.csv"):
        print("Pose data preprocessing...")
        ## Pose estimation
        if list(os.listdir(f"{keypoints_path}/{video_name}")) == []:
            print("Pose estimation...")
            video_to_keypoints(os.path.abspath(input_path), os.path.abspath(keypoints_path))
            tf.keras.backend.clear_session()
        keypoints_dir = os.path.abspath(os.path.join(keypoints_path, video_name))
        record_pose_df = keypoints_to_data(
            keypoints_dir, os.listdir(f"{keypoints_path}/{video_name}"), record_pose_path
        )
        print("=========================================")
        print(f"Data have been saved in {record_pose_path}")
        print("=========================================")
    else:
        record_pose_df = pd.read_csv(f"{record_pose_path}/{video_name}.csv")
        ## 將文字骨幹資料轉為數值資料
        # trans_col = [col for col in usecols[2:]]
        # record_pose_df[trans_col] = record_pose_df[trans_col].apply(string_to_numeric)
        # record_pose_df[['Frame','Person_id']] = record_pose_df[['Frame','Person_id']].astype(int)
        print("=========================================")
        print(f"Keypoints already exist in {keypoints_path}")
        print(f"Data already exist in {record_pose_path}")
        print("=========================================")

    end = time.time()
    print(f"Pose time: {end-start} seconds.")
    total_time += end - start

    start = time.time()
    ## TrackNetV2 prediction
    if not os.path.exists(f"{record_ball_path}/{video_name}_ball.csv"):
        print("Ball prediction...")
        cap = cv2.VideoCapture(f"{input_path}")
        ...
        count = 0
        image_list = [0 for _ in range(12)]
        frame_time_list = [0 for _ in range(12)]
        image_x_list = [0 for _ in range(12)]

        model = load_model(weight_ball, custom_objects={"custom_loss": Custom_Loss})
        # model.summary()
        ...
        frame_time_list[0] = frame_time

        for i in range(1, 12):
            success, image_list[i] = cap.read()
            frame_time_list[i] = Custom_Time(cap.get(cv2.CAP_PROP_POS_MSEC))
        ...
        while success:
            ...
            for i in range(12):
                ...
                ## 12_in_12_out data prepare #########################
                image_x_list[i] = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
                image_x_list[i] = cv2.resize(image_x_list[i], (WIDTH, HEIGHT), interpolation=cv2.INTER_CUBIC)
                image_x_list[i] = image_x_list[i].reshape((image_x_list[i].shape[0], image_x_list[i].shape[1], 1))
                unit.append(image_x_list[i])
                ######################################################
            ## TrackNetv2 predict ################################
            unit = np.asarray(unit)
            unit = unit.reshape((1, 12, HEIGHT, WIDTH))
            ...
            isball = []
            for i in range(12):
                frame_time = frame_time_list[i]
                # 沒偵測到球
                if np.amax(h_pred[i]) <= 0:
                    record_ball_df.loc[len(record_ball_df.index)] = [int(count), 0, 0, 0, frame_time]
                # 有偵測到球
                else:
                    ...
                    # 記錄
                    record_ball_df.loc[len(record_ball_df.index)] = [int(count), 1, x_c_pred, y_c_pred, frame_time]
            ...
            # image_list.pop(0)
            # frame_time_list.append(Custom_Time(cap.get(cv2.CAP_PROP_POS_MSEC)))
        ...
        #     )
        # new_ball_df = pd.DataFrame(columns=record_ball_df.columns)
        # data = pd.DataFrame(columns=record_ball_df.columns)
        # for index, row in record_ball_df.iterrows():

        #     if end_flag and row['Visibility']==1:
        #         start_flag = True
        #         end_flag = False
        #         start_idx = index
        #     elif start_flag and row['Visibility'] == 0:
        #         end_flag = True
        #         start_flag = False
        #         end_idx = index - 1

        #         data = record_ball_df.loc[start_idx:end_idx].copy()

        #         coord = data[['X','Y']].copy()
        #         if len(coord) > 3:
        #             zcore = ((coord - coord.mean()) / coord.std()).abs() > 3
        #             coord[zcore == True] = np.nan
        #             coord = imp_mean.fit_transform(coord).astype(np.int64)
        #             data.loc[:,['X','Y']] = coord

        #         new_ball_df = pd.concat([new_ball_df, data])
        #         new_ball_df.loc[len(new_ball_df.index)] = row
        #     elif end_flag and row['Visibility'] == 0:
        #         new_ball_df.loc[len(new_ball_df.index)] = row
        # record_ball_df = new_ball_df

        # 計算曲率
        x = record_ball_df["X"].to_numpy().astype(np.int)
        y = record_ball_df["Y"].to_numpy().astype(np.int)
        # ka = []
        # for idx in range(2, len(x)):
        #     cur_x = x[idx-2:idx+1]
        #     cur_y = y[idx-2:idx+1]
        #     xy = list(zip(cur_x, cur_y))
        #     if xy.count((0, 0)) == 1 and xy.index((0,0)) == 1:
        #         ka.append(0)
        #         continue
        #     if ka != []:
        #         if xy.count((0, 0)) >= 2:
        ...
        #                 continue
        #             elif idx_xy == 1:
        #                 cur_x[1] = int((cur_x[2] + cur_x[0])/2)
        #                 cur_y[1] = int((cur_y[2] + cur_y[0])/2)
        #     kappa, _ = PJcurvature(cur_x, cur_y)
        #     if kappa < -1.0:
        #         kappa = -1.0
        #     elif kappa > 1.0:
        #         kappa = 1.0
        ...
        a_1 = [0, 0, 0, 0, 0]
        a_0 = [0, 0, 0, 0, 0]
        for idx in range(5, len(x) - 3):
            poly_x = x[idx - 5 : idx + 4]
            poly_y = y[idx - 5 : idx + 4]
            ...
            if xy.count((0, 0)) >= 6:
                ...
                a_0.append(0)
                continue
            poly_z = np.polyfit(poly_x, poly_y, 1)
            # a_2.append(poly_z[0])
            a_1.append(poly_z[0])
            a_0.append(poly_z[1])
        for _ in range(3):
            ...
            a_1.append(0)
            a_0.append(0)
        # record_ball_df['a_2'] = a_2
        record_ball_df["a_1"] = a_1
        record_ball_df["a_0"] = a_0
        # ka = [np.nan, np.nan] + ka
        # record_ball_df['Curvature'] = ka
        ## MinMax scale
        # record_ball_df['Curvature'] = (record_ball_df['Curvature'] -record_ball_df['Curvature'].min()) / (record_ball_df['Curvature'].max() - record_ball_df['Curvature'].min())
        # record_ball_df['a_2'] = (record_ball_df['a_2'] - record_ball_df['a_2'].min())/ (record_ball_df['a_2'].max() - record_ball_df['a_2'].min())
        record_ball_df["a_1"] = (record_ball_df["a_1"] - record_ball_df["a_1"].min()) / (
            record_ball_df["a_1"].max() - record_ball_df["a_1"].min()
        )
        record_ball_df["a_0"] = (record_ball_df["a_0"] - record_ball_df["a_0"].min()) / (
            record_ball_df["a_0"].max() - record_ball_df["a_0"].min()
        )
        record_ball_df["Ball_X"] = (record_ball_df["X"] - record_ball_df["X"].min()) / (
            record_ball_df["X"].max() - record_ball_df["X"].min()
        )
        record_ball_df["Ball_Y"] = (record_ball_df["Y"] - record_ball_df["Y"].min()) / (
            record_ball_df["Y"].max() - record_ball_df["Y"].min()
        )
        # record_ball_df['ball'] = list(zip(record_ball_df['X'], record_ball_df['Y']))
        # record_ball_df.drop(columns=["X", "Y"], inplace=True)

        record_ball_df.to_csv(f"{record_ball_path}/{video_name}_ball.csv", index=False)

    else:
        # 讀取桌球軌跡紀錄
        record_ball_df = pd.read_csv(f"{record_ball_path}/{video_name}_ball.csv")
        # record_ball_df['ball'] = record_ball_df['ball'].apply(lambda x: re.sub("[(,)]", "", x))
        # record_ball_df['ball'] = record_ball_df['ball'].apply(lambda x: [float(y) for y in x.split(" ")])
        print("=========================================")
        print(f"Ball trajectory already exist in {record_ball_path}")
        print("=========================================")

    end = time.time()
    print(f"Ball time: {end-start} seconds.")
    total_time += end - start

    start = time.time()
    # Table tennis acton classification
    if not os.path.exists(f"{output_path}/{video_name}_predict_class.csv"):
        print("Table tennis acton classification...")
        if not os.path.exists(f"{output_path}/{video_name}_predict_pose.csv"):
            print("Action predicting...")
            ## Prepare test data
            ## 結合 Pose 及 Ball Trajectory
            test_df = pd.merge(record_ball_df, record_pose_df, on="Frame", how="inner")
            test_df = test_df.sort_values(by=["Person_id", "Frame"])
            test_df.fillna(method="bfill", inplace=True)
            test_df[window_usecols] = Normalize_df(test_df, window_usecols)
            # test_df = record_pose_df

            model = load_model(f"{weight_op}")
            # model_op.summary()
            ...
            for p_id in pd.unique(test_df["Person_id"]):
                test_data = test_df[test_df["Person_id"] == p_id]
                for range_idx in range(n * k, len(test_data) + 1 - n * k):
                    item = np.array(
                        test_data.iloc[range_idx - n * k : range_idx + n * k : n][window_usecols]
                        .values.flatten()
                        .tolist()
                    )
                    if len(item) % batch != 0:
                        continue
                    item = item.reshape(-1, batch, len(window_usecols) * 1)

                    pred = model.predict(item)
                    if p_id == 0:
                        ans["Player_1"].append(np.argmax(pred, axis=1)[0])
                    elif p_id == 1:
                        ans["Player_2"].append(np.argmax(pred, axis=1)[0])

            tf.keras.backend.clear_session()

            ans_df = pd.DataFrame(ans)

            ans_df.to_csv(f"{output_path}/{video_name}_predict_pose.csv", index=False)

            print("=========================================")
            print(f"Prediction has been saved in {output_path}")
            print("=========================================")
        else:
            ans_df = pd.read_csv(f"{output_path}/{video_name}_predict_pose.csv")
            print("=========================================")
            print(f"Prediction already exists in {output_path}")
            print("=========================================")
        ans_df["class_1"] = predict_class(ans_df["Player_1"], 10)
        ans_df["class_2"] = predict_class(ans_df["Player_2"], 10)
        ans_df.to_csv(f"{output_path}/{video_name}_predict_class.csv", index=False)

        print("=========================================")
        print(f"Class has been saved in {output_path}")
        print("=========================================")
    else:
        ans_df = pd.read_csv(f"{output_path}/{video_name}_predict_class.csv")
        print("=========================================")
        print(f"Class already exists in {output_path}")
        print("=========================================")

    end = time.time()
    print(f"Action time: {end-start} seconds.")
    total_time += end - start
    ...
    while success:
        ...
        if count >= k:
            ...
            if np.count_nonzero(temp_ball) > k / 2:
                ...
                if (is_A_serve or is_B_serve) and (
                    ...
                    # print(f"------>Frame : {count} Catch serve detection fault!!<------\n")
        ...
        image_CV = image.copy()
        ## 沒偵測到球體
        if record_ball_df.loc[count, "Visibility"] == 0:
            pass
        ## 有偵測到球體
        else:
            x_c_pred, y_c_pred = record_ball_df.loc[count, ["X", "Y"]]
