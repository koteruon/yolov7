...
### 後處理從此開始 ###
def main(root_path, video_name):
    # 全域參數
    HEIGHT = 288  # model input size
    WIDTH = 512

    # 影片跟目錄
    input_path = os.path.join(root_path, video_name)
    ...
    # 設定過一次就不用再設定
    PT_data["point_x"] = [484, 398, 1504, 1469]
    PT_data["point_y"] = [686, 832, 806, 660]
    # 第一次設定桌子的四個角
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root_path", type=str, default="yolo7.pt", help="initial weights path")
    parser.add_argument("--video_name", type=str, default="", help="model.yaml path")
    opt = parser.parse_args()
    main(opt.root_path, opt.video_name)
