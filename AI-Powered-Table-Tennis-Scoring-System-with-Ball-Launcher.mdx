...
- **IoU score**: $S_{\text{iou}} = \text{IoU}$
- **Direction score** (cosine similarity bounded in $[-1, 1]$): $S_{\text{dir}} = \cos(\theta) = \frac{\vec{v}_{\text{avg}} \cdot \vec{v}_{\text{new}}}{\| \vec{v}_{\text{avg}} \| \cdot \| \vec{v}_{\text{new}} \|}$
- **Distance score** (normalized and reversed): $S_{\text{dis}} = 1 - \min\left( \frac{d}{d_{\text{max}}}, 1 \right)$
- **Aspect Ratio score**: $S_{\text{ar}} = 1 - \min\left( \frac{|\frac{w}{h} - \frac{w'}{h'}|}{\Delta_{\text{max}}}, 1 \right)$
